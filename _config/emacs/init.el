...
;; Icon fonts. To install the resource fonts run the command
...
(defun jm/c-mode-common-defaults ()
  (setq c-basic-offset 4
        c-default-style "linux")
  (c-set-offset 'substatement-open 0))

(add-hook 'c-mode-common-hook 'jm/c-mode-common-defaults)

;; OCaml - Major mode provided by `tuareg' and context sensitive completion
;; with `merlin'.
(use-package tuareg
  :custom (tuareg-match-patterns-aligned t))

;; Use the opam installed `merlin'.
...
;; Make
(defun jm/makefile-mode-defaults ()
  ;; Show whitespace when in `makefile-mode'.
  (whitespace-toggle-options '(tabs)))

(add-hook 'makefile-mode-hook 'jm/makefile-mode-defaults)
...
;; Better way to run code formatters on save.
(use-package apheleia
  ...
  :custom
  (apheleia-formatters '((ocamlformat . ("ocamlformat" "--enable-outside-detected-project" "-"))
                         (mixformat . ("mix" "format" "-"))
                         (rustfmt . ("rustfmt" "--edition" "2018"))))
  (apheleia-mode-alist '((tuareg-mode . ocamlformat)
                         (elixir-mode . mixformat)
                         (rust-mode . rustfmt))))

;; Lower garbage collection values when idle for 5 seconds.
(run-with-idle-timer
 5 nil
 (lambda ()
